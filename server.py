    ...
    try:
        ...
        if comanda == 'CONNECT':
            ...
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Trebuie sa fii conectat pentru a folosi PUBLISH."
            elif not argumente:
                raspuns = "EROARE: Mesajul nu poate fi gol."
            else:
                mesaje[urmatorul_id] = {"text": argumente, "autor": adresa_client}
                raspuns = f"OK: Mesaj publicat cu ID={urmatorul_id}"
                urmatorul_id += 1

        elif comanda == 'DELETE':
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Trebuie sa fii conectat pentru a folosi DELETE."
            else:
                try:
                    ...
                    if id_mesaj in mesaje:
                        if mesaje[id_mesaj]["autor"] == adresa_client:
                            del mesaje[id_mesaj]
                            raspuns = "OK: Mesajul a fost sters."
                        else:
                            raspuns = "EROARE: Nu poti sterge decat propriile mesaje."
                    else:
            ...
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Trebuie sa fii conectat pentru a folosi LIST."
            else:
                if not mesaje:
                    raspuns = "OK: Nu exista mesaje publicate."
                else:
                    raspuns = "OK: Mesaje:\n" + "\n".join([f"ID={k}: {v['text']}" for k, v in mesaje.items()])
