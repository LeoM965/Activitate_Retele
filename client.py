...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            print("EROARE: Trebuie sa fii conectat pentru a trimite comenzi la server.")
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Sintaxa gresita. Foloseste: PUBLISH <mesaj>")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            ...
            continue
        if len(parti) < 2 or not parti[1].strip():
            print("EROARE: Sintaxa gresita. Foloseste: DELETE <id>")
            continue
        try:
            int(parti[1].strip())
        except ValueError:
